...
It includes the device IOCTL definitions (using IOC_MAGIC method to avoid IOCTL numbering conflicts ),
also includes user space ioctl wrapper functions.  In the unlikely event where you need to use one of the IOCTL commands, you might want to borrow this code to speed up the application.
...
//*****************************************************
#ifndef __KERNEL__
int set_counter( int counter_dev, int counter_offset, int counter_value, int counter_mode )
{
    ...
    ioctl(counter_dev, COUNTER_IOC_SET_COUNTER, test_counter_params );
    return 0;
}

void reset_counter( int counter_dev )
{
    //reset the counters ??? **CHECK THIS **
    ioctl(counter_dev, COUNTER_IOC_RESET ); 
}

int read_counter_latch( int counter_dev, int counter_offset )
{
    int test_counter_params[6];
    test_counter_params[0] = counter_offset;
    
    ioctl(counter_dev, COUNTER_IOC_COUNTER_LATCH, test_counter_params );
