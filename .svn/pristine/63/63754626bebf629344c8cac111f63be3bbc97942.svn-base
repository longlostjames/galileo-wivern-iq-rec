...
void RDQ_InitialiseISACTRL (int pulses_per_daq_cycle, int samples_per_pulse, int clock_divfactor, int delay_clocks)
...
void RDQ_StartAcquisition( int amcc_fd, int dma_bank, short int *data, int tcount )
{
 	...
 	*/
        ...
        /* initialise the dma_bank so it is obvious if we run into problems */
        /* 0xFFFF cannot be valid since the maximum ADC value will be 0x0FFF */
        for (i = 0; i < (tcount/(sizeof (short int))); i++)
        {
                data[i] = 0xFFFF;
        }
