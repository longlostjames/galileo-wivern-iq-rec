...
void RAT_CheckError( I16 card, I16 err, char *buffer)
{
if (err != 0) {
...
I16 RAT_StartAcquisition( int pulses, int samples, I16 *data_buffer )
{
...
err = D2K_AI_Config (card,
        ...
        DAQ2K_AI_TRGSRC_ExtD | DAQ2K_AI_TRGMOD_POST | DAQ2K_AI_TrgNegative | DAQ2K_AI_ReTrigEn,
        0,
        0,
...
err = D2K_AI_ContBufferSetup (card, data_buffer, samples * pulses * channels, &Id);
RAT_CheckError(card, err, "AI_ContBufferSetup error");

/* start the acqusiiton off */ 
err = D2K_AI_ContReadMultiChannels(card, channels, channel, Id, samples * pulses, sample_intrval, sample_intrval, ASYNCH_OP);
