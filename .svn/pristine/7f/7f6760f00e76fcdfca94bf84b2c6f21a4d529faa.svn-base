...
# MOD
# now pass the date/time from bowzer
...
while(1)
{
	...
	$program = "/usr/local/bin/radar-galileo-rec";
	...
	print("Waiting for a UDP packet from bowzer (this should indicate the start of a scan)\n\n");
	while ($udp_length != 64) 
	{
		recv( $sock, $ir, 64, 0);
	...
	$experiment_id = ord(substr($ir,36,1));
	...
	if ($child != 0)
	{
		...
		if ($scan_type eq "FIX") 
   		{
			$command = $program." -".lc($scan_type)." 3600"." ".$min_angle." ".$max_angle." -scan_angle ".$scan_angle; 
			$command .= " -sv ".$velocity;
   		} elsif ($scan_type ne "NOT_KNOWN") {
   			$command  = $program." -".lc($scan_type)." ".$min_angle." ".$max_angle." -scan_angle ".$scan_angle;
			$command .= " -sv ".$velocity;
   		}
                $more_flags = " -op ".$operator." ";
 		# if ($psd > 0) { $more_flags .= "-spec $psd "; }
		$more_flags .= "-id $experiment_id -file $file -scan $raster -date $yyyymmddhhmmss ";
		...
		print "\007";
        	...
        	print "     CONTROL BLOCK PARAMETERS\n";
        	print "------------------------------------\n";
        	print "ACROBAT RECORDING   : OFF\n";
		print "------------------------------------\n";
		print "Do you to ACROBAT ON on bowzer?\n";
	}
