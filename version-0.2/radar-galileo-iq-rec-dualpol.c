...
// Development Version
// ---------------------------------------------------------------
// Acquisition software for dualpol IQ data from 94-GHz radar
//
...
#include "/home/chilbolton_software/radar-galileo/radar-galileo-iq-rec/version-0.2/universal_radar_code/RDQ/include/RDQ.h"
// Include file for the RNC package
#include "/home/chilbolton_software/radar-galileo/radar-galileo-iq-rec/version-0.2/universal_radar_code/RNC/include/RNC.h"
// Include file for the REL package
#include "/home/chilbolton_software/radar-galileo/radar-galileo-iq-rec/version-0.2/universal_radar_code/REL/include/REL.h"
// Include file for the RSM package
#include "/home/chilbolton_software/radar-galileo/radar-galileo-iq-rec/version-0.2/universal_radar_code/RSM/include/RSM.h"
...
void disp_welcome_message(void)
{
  ...
  printf("\n");
  printf("radar-galileo-iq-rec-dualpol: Version %s\n\n",buffer);
}
...
FILE *    pFile = NULL;
char      filename[255];
char      configfile[255]="/mnt/focus_radar_data/radar-galileo/etc/radar-galileo-iq-dualpol.conf";
int       num_pulses;
...
I_uncoded_copolar_V        = (short int *)malloc(sizeof(short int) * num_data);
Q_uncoded_copolar_V        = (short int *)malloc(sizeof(short int) * num_data);
IQStruct.I_uncoded_copolar_V = (short int *)malloc(sizeof(short int) * param.samples_per_pulse * param.nfft * param.spectra_averaged);
IQStruct.Q_uncoded_copolar_V = (short int *)malloc(sizeof(short int) * param.samples_per_pulse * param.nfft * param.spectra_averaged);
	...
	gettimeofday( &tv, &tz);
      ...
      if (newfile==1) {
           if (pFile != NULL)
            ...
            sprintf(filename,"/mnt/focus_radar_data/radar-galileo/raw/ts/%s_iqdata-dualpol.bin",datestring);
	    	...
	    	for (i = 0; i < num_pulses; i++) 
		{
	      		...
	      		for (j = 0; j < param.samples_per_pulse; j++) 
			{
				...
				I_uncoded_copolar_V[count_reg] = GET_CHANNEL( data, CHAN_Ix );
				Q_uncoded_copolar_V[count_reg] = GET_CHANNEL( data, CHAN_Qx );
        ...
        fwrite(IQStruct.I_uncoded_copolar_V , sizeof(short), total_samples, pFile);
        fwrite(IQStruct.Q_uncoded_copolar_V , sizeof(short), total_samples, pFile);
