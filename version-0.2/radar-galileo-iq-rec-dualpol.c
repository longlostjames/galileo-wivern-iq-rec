...
/// Based on code base developed by John Nicol (Univ. Reading)
...
/* 1 is /dev/ttyS0 or COM1 in MS-DOS language */
...
static void SetupTimeSeriesVariables (
		TimeSeriesObs_t           *obs,
		int                        ncid,
...
static void WriteOutTimeSeriesData (
		int                        ncid,
...
int main(int argc, char *argv[])
{
	...
	uint16_t     *dma_banks[2];      // pointers to the dma banks anks */
	int           dma_bank = 0;
	int           proc_bank = 1;
		...
		if (new_mode >= 0)
		{
			...
			for (i = 0; i < num_pulses; i++)
			{
				...
				for (j = 0; j < param.samples_per_pulse; j++)
				{
					...
					if (swap_iq_channels)
					{
						I_uncoded_copolar_H[count_reg]    = GET_CHANNEL (data, SWAP_CHAN_Ic);
						Q_uncoded_copolar_H[count_reg]    = GET_CHANNEL (data, SWAP_CHAN_Qc);
						I_uncoded_crosspolar_H[count_reg] = GET_CHANNEL (data, SWAP_CHAN_Ix);
						Q_uncoded_crosspolar_H[count_reg] = GET_CHANNEL (data, SWAP_CHAN_Qx);
					}
					else
					{
						I_uncoded_copolar_H[count_reg]    = GET_CHANNEL (data, CHAN_Ic);
